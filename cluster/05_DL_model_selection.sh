...
#SBATCH --output=../logs/05_run_DLModels_%j.out
#SBATCH --error=../logs/05_run_DLModels_%j.err
...
# --- Config selection (passed as first argument) ---
CONFIG=$1
if [ -z "$CONFIG" ]; then
  echo "ERROR: no config given. Usage: sbatch 04_run_DL_model.sh <config.yaml>"
  exit 1
fi
