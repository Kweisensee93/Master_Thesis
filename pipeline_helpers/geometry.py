...
def landmark_anchors(
    subset: np.ndarray,
    landmarks: list[tuple],
    crop_box: tuple,
) -> np.ndarray:
    """
    Extends a contour subset at both ends to reach the true landmark positions,
    rather than the nearest snapped contour point.

    Parameters
    ----------
    subset    : np.ndarray (M, 2) — contour subset in crop space (from get_contour_subset).
    landmarks : list of (x, y) tuples in full image space (already y-flipped).
    crop_box  : (min_x, max_x, min_y, max_y) — defines the crop origin offset.

    Returns
    -------
    np.ndarray (M+2, 2) — subset extended with true landmark positions at both ends.
    """
    first_lm, last_lm = to_crop_space([landmarks[0], landmarks[-1]], crop_box)
    first_lm = np.array(first_lm, dtype=np.float32).reshape(1, 2)
    last_lm  = np.array(last_lm,  dtype=np.float32).reshape(1, 2)

    return np.concatenate([first_lm, subset, last_lm])
