...
# for now we import each function directly
# later we can cut it down to #from pipeline_helpers import *
...
GetImages = False   # Set to True to enable images with contour and landmarks drawn
...
if __name__ == "__main__":
    ...
    for img_name in fish_images:
        ...
        # Step 2: Convert cropped image to bytes and remove background
        ...
        # --- Step 6: Define cropped_bgr and Draw (The Missing Definition) ---
        # Draw the continuous contour line in green
        cv2.drawContours(cropped_bgr, [fish_contour_crop], -1, (0, 255, 0), 2)
        
        contour_img_path = OUTPUT_DIR / f"{stem}_contour.png"
        cv2.imwrite(str(contour_img_path), cropped_bgr)
        if AddInfo: print(f"  Contour image saved: {contour_img_path}")

        # Step 7: Save contour points to CSV
        ...
        # Step 8: Find closest contour points to first and last landmark
        ...
        # Resample to 10 equidistant points along the contour (semilandmarks_new)
        semilandmarks_new_crop = resample_points(subset_crop, num_points=10)

        # ── Draw on image ──
        # Anchor points – red
        cv2.circle(cropped_bgr, closest_first_crop, radius=6, color=(0, 0, 255), thickness=-1)
        cv2.circle(cropped_bgr, closest_last_crop,  radius=6, color=(0, 0, 255), thickness=-1)
        # semilandmarks_new – yellow
        for pt in semilandmarks_new_crop[1:-1]:  # skip anchors, already drawn
            cv2.circle(cropped_bgr, tuple(pt.astype(int)), radius=5, color=(0, 255, 255), thickness=-1)
        cv2.imwrite(str(contour_img_path), cropped_bgr)
...
# ── Step 9: Calculate Euclidean Distances ──────────────────────────────────────

        # 1. Get Ground Truth (GT) semilandmarks from TPS (full image space)
        ...
        # 3. Convert semilandmarks_new_crop fromcrop space → full image space
        semilandmarks_new_full = semilandmarks_new_crop + np.array([ox, oy], dtype=np.float32) # already in crop space = full space now

        # 4. Convert contour from resized-crop space → full image space
        ...
        # 7. Save updated visualisation with pred landmarks in full-image space
        #    re-mapped back to cropped space for drawing
        for pt in semilandmarks_new_crop[1:-1]:   # skip anchors, already drawn
            cv2.circle(cropped_bgr, tuple(pt.astype(int)), radius=5, color=(255, 0, 255), thickness=-1)
        cv2.imwrite(str(contour_img_path), cropped_bgr)
