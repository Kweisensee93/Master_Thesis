...
# Densify contour_full before passing to calculate_metrics
def densify_contour(contour: np.ndarray, step: float = 1.0) -> np.ndarray:
    """Resample contour to ~1px spacing."""
    diffs = np.diff(contour, axis=0)
    seg_lens = np.sqrt((diffs**2).sum(axis=1))
    arc = np.concatenate([[0], np.cumsum(seg_lens)])
    targets = np.arange(0, arc[-1], step)
...
if __name__ == "__main__":
    ...
    GetContour                        = cfg.get("GetContour",                        False) # Set to True to enable output of contour points as CSV
    ...
    #fish_images = fish_images[3::4] # DO REMOVE THIS LINE FOR FULL RUN!!!!!
    if DefinedFile != "all":
    ...
    for img_name in fish_images:
        ...
        if invalid_landmarks:
            print(f"WARNING: Found {len(invalid_landmarks)} out-of-bounds landmarks in {img_name}, skipping this image.")
            continue
        ...
        subset_crop[:, 0] = savgol_filter(subset_crop[:, 0], savgol_filter_window, polyorder=3)
        subset_crop[:, 1] = savgol_filter(subset_crop[:, 1], savgol_filter_window, polyorder=3)
        

        # x_smooth, y_smooth = subset_crop[:, 0], subset_crop[:, 1]
        # # s controls smoothing strength — increase if still too rough
        # s = len(subset_crop)
        # tck, u = splprep([x_smooth, y_smooth], s=s, per=False)
        # x_smooth, y_smooth = splev(np.linspace(0, 1, len(subset_crop)), tck)
        # subset_crop = np.column_stack([x_smooth, y_smooth]).astype(np.float32)
        ...
        contour_full_dense = densify_contour(contour_full)
        img_results, img_metrics_df = calculate_metrics(
            img_name, semilandmarks_new_crop, gt_semi_full, contour_full_dense, ox, oy, getinfo=GetInfo
            )
        # img_results, img_metrics_df = calculate_metrics(
        ...
        export_results(geometry_results, OUTPUT_DIR, GetInfo, GetImages, DefinedFile, debug_01, get_contour=GetContour)
